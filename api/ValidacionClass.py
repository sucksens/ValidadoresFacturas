    ...
    def obtener_estado(self, rfc_emisor, rfc_receptor, total, uuid):
        ...
        if response.status_code != requests.codes["ok"]:
            if not response.text.startswith("<s:Envelope"):
                ...
                ex = resp_xml.find(
                    "s:Body/s:Fault/faultstring", namespaces=self.NSMAP
                ).text
            raise Exception(ex)
        ...
        f_val = "s:Body/t:ConsultaResponse/t:ConsultaResult/a:EstatusCancelacion"
        EstatusCancelacion = resp_xml.find(f_val, namespaces=nsmap)

        f_val = "s:Body/t:ConsultaResponse/t:ConsultaResult/a:ValidacionEFOS"
        ValidacionEFOS = resp_xml.find(f_val, namespaces=nsmap)

        ret_val = {
            "codigo_estatus": CodigoEstatus.text,
            "es_cancelable": EsCancelable.text,
            "estado": Estado.text,
            "estatus_cancelacion": EstatusCancelacion.text
            if EstatusCancelacion is not None
            else None,
            "validacion_efos": ValidacionEFOS.text
            if ValidacionEFOS is not None
            else None
        }
        return ret_val
